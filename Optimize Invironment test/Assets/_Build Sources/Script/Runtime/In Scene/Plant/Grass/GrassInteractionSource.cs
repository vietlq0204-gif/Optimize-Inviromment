...
public sealed class GrassInteractionSource : MonoBehaviour
{
    ...
    public float PlanarHalfScale => Mathf.Max(Mathf.Abs(transform.lossyScale.x), Mathf.Abs(transform.lossyScale.z)) * 0.5f;
    ...
    public void UpdateWakeState(float deltaTime, float wakeLengthPerSpeed, float wakeBuildDistanceMultiplier, float wakeDirectionResetDot)
    {
