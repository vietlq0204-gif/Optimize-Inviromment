Shader "Custom/Vit/Plant_URP"
{
    ...
    SubShader
    {
        ...
        {
            PlantSurfaceSample sample = SamplePlantSurface(uv);
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                PlantSurfaceSample surface = SamplePlantSurface(input.uv);
                half alpha = surface.baseAlpha;
                clip(alpha - _Cutoff);

                float colorEnabled = ColorEnabled01();
                float3 baseColorTint = lerp(float3(1.0, 1.0, 1.0), _BaseColor.rgb, colorEnabled);
                float3 color = surface.baseTex.rgb * baseColorTint;
