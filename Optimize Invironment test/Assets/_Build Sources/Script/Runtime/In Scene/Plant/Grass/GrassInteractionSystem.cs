...
/// <summary>
/// Writes grass interaction shapes into the global grass interaction map.
/// </summary>
[ExecuteAlways]
[AddComponentMenu("Grass/Grass Interaction System")]
public class GrassInteractionSystem : EnvironmentInteractionSystemBase
{
    ...
    [SerializeField] protected Vector3 worldOffset = Vector3.zero;
    ...
    public override InteractionTargetMask ConsumedTargets => InteractionTargetMask.Vegetation;
    ...
    protected virtual void EnsureInteractionTextures()
    {
        ...
        interactionTexture = EnsureTexture(interactionTexture, descriptor, "GrassInteractionRT");
        interactionScratchTexture = EnsureTexture(interactionScratchTexture, descriptor, "GrassInteractionScratchRT");
    }
    ...
    protected virtual void EnsureHistoryTextures()
    {
        ...
        interactionHistoryA = EnsureTexture(interactionHistoryA, descriptor, "GrassInteractionHistoryA");
        interactionHistoryB = EnsureTexture(interactionHistoryB, descriptor, "GrassInteractionHistoryB");
    }
    ...
    protected virtual void EnsureBatchStampMaterial()
    {
        ...
        if (shader == null)
        {
            Debug.LogWarning("GrassInteractionSystem could not find shader 'Hidden/Vit/EnvironmentInteractionBatch'.", this);
            return;
        }
    ...
    protected virtual bool CollectInteractionShapes(Vector3 regionCenter)
    {
        ...
        InteractionTargetMask consumedTargets = ConsumedTargets;
        for (int i = collectedShapes.Count - 1; i >= 0; i--)
        {
            if ((collectedShapes[i].Targets & consumedTargets) != 0)
            {
                continue;
            }
    ...
    protected virtual int FillShapeBatch(int startIndex, int batchCount)
    {
        ...
        for (int i = startIndex; i < endIndex; i++)
        {
            ...
            if ((shape.Targets & ConsumedTargets) == 0)
            {
                continue;
            }
